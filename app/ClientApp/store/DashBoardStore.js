...
export const actionCreators = {
        ...
        request.get('/api/v1/post/')
            ...
            .then(res => {
                if (res.ok) {
                    ...
                    dispatch({ type: 'DASHBOARD_GETUSER_POSTS_FINISHED', payload: { myPostData: data } });
                }
            })
            .catch(err => {
                dispatch({ type: 'DASHBOARD_GETUSER_POSTS_ERR', payload: 'Failed to get post' });
            });
    },
    onEditorChanged: (e) => (dispatch, getStore) => dispatch({ type: 'DASHBOARD_EDITOR_CHANGED', payload: e }),
    onTitleChanged: (e) => (dispatch, getStore) => dispatch({ type: 'DASHBOARD_TITLE_CHANGED', payload: e.target.value }),
    onExcerptChanged: (e) => (dispatch, getStore) => dispatch({ type: 'DASHBOARD_EXCERPT_CHANGED', payload: e.target.value }),
    onPublishBtnClicked: () => (dispatch, getStore) => { 
        var { title, excerpt, editorState, _id } = getStore().dashboard,
            _url;
        if (isEmpty(title)) {
            dispatch({ type: 'DASHBOARD_TITLE_NULL', payload: `Title can't be null.` });
            return;
        }
        if (isEmpty(excerpt)) {
            dispatch({ type: 'DASHBOARD_EXCERPT_NULL', payload: `Excerpt can't be null.` });
            return;
        }
        if (!_id)
        ...
        dispatch({ type: 'DASHBOARD_PUBLISH_STARTED' });
        request.post(_url)
               .send({ 
                   title: title, 
                   excerpt: excerpt, 
               ...
               .then(res => {
                    dispatch({ type: 'DASHBOARD_PUBLISH_FINISHED', payload: {
                        title: '',
                        excerpt: '',
                        editorState: EditorState.createEmpty(),
               ...
               .catch(err => {
                    dispatch({ type: 'DASHBOARD_PUBLISH_ERR', payload: err });
               });
    },
    onDraftBtnClicked: (e) => (dispatch, getStore) => dispatch({ type: 'DASHBOARD_DRAFT_CLICKED', payload: e }),
    onDiscardBtnClicked: () => (dispatch, getStore) => {
        var { _id } = getStore().dashboard;
        if (!_id) {
            dispatch({ type: 'DASHBOARD_DISCARD_CLICKED', payload: emptyEditor }); 
            return;
        }
        ...
        request.del(`/api/v1/post/${_id}/remove`)
               ...
               .catch(err => {
                   console.log(err);
                   dispatch({ type: 'DASHBOARD_DISCARD_ERR', payload: { editorMsg: 'Failed to remove the post.' } });
               });
    },
    onPreviewBtnClicked: (e) => (dispatch, getStore) => dispatch({ type: 'DASHBOARD_PREVIEW_CLICKED', payload: e }),
    getPosts: (page, who) => (dispatch, getStore) => { 
        ...
        request.get(`/api/v1/post?p=${page}&w=${who}&_t=${window.localStorage._t}`)
               ...
               .catch(err => {
                    dispatch({ type: 'DASHBOARD_GET_POSTS_ERR', payload: { myPostMsg: err } });
               });
    },
    onEditBtnClick: (event, props) => (dispatch, getStore) => {
        var { _id } = props;
        dispatch({ type: 'DASHBOARD_GET_EDIT_POST_STARTED' });
        request.get(`/api/v1/post/${_id}/edit`)
               ...
               .catch(err => {
                   dispatch({ type: 'DASHBOARD_GET_EDIT_POST_ERR', payload: { myPostMsg: err } });
               });
    },
}
...
export const reducer = (state, action) => {
    switch (action.type) {
        ...
        case 'DASHBOARD_GETUSER_POSTS_ERR':
            return { ...state, myPostMsg: action.payload };
        ...
        case 'DASHBOARD_GET_EDIT_POST_ERR':
            return { ...state, editorMsg: action.payload };
